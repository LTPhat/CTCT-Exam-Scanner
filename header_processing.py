...
def get_MSSV_by_model(model, header_img):
    """
...
def get_mssv_by_processing(model, header_img):
    """
    ...
    mssv_line_height = math.ceil(mssv_img.shape[0] // 11)
    mssv_line_width = math.ceil(mssv_img.shape[1] // 7)

    digit_list = []
    for i in range(7):
        # Get a column
        col_img = mssv_img[:, i * mssv_line_width : (i + 1) * mssv_line_width]
        # Get bubles each column
        for j in range(11):
            # Ignore handwriting bubble
            if j == 0:
                continue
            digit_img = col_img[j * mssv_line_height: (j+1) * mssv_line_height, :]
            digit_img = cv2.threshold(digit_img, 0, 255, cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)[1]
